...
if [ -z "`dpkg -l | grep clamav`" ]
	then gnome-terminal --hide-menubar --geometry=80x15 -t "Установка пакета clamav" -- bash -c "echo \"clamav не установлен\"; echo; \
...
if [ $? = 0 ]
	then
		...
		gnome-terminal --hide-menubar --geometry 110x30 -t "clamav $PWD" -- bash -c "$update_command echo \"Идет загрузка базы сигнатур\"; echo; \
